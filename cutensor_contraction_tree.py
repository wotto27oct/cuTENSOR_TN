...
for r in range(rnum):
    ...
    print(f"calculation time for optimal contraction tree: {cp.cuda.get_elapsed_time(start, end) /1000}[s]")
    top = Q_best[all]
    start = stream.record()
    top.calc_tensor()
    end = stream.record()
...
if naive == 1:
    ...
    print(f"cutensor naive: ave {cp.average(cp_naive_list)}, std {cp.std(cp_naive_list)}")

cp_optimal_list = cp.array(cp_optimal_list)
print(f"cutensor optimal: ave {cp.average(cp_optimal_list)}, std {cp.std(cp_optimal_list)}")
